...
// Deep polar-night sky with a subtle vertical gradient plus a faint horizon glow.
		...
		// each clip's poster frame; capped at 1080p — 4K files stutter when decoded
		// fullscreen.
		const VIDEO_PRESETS = [
			{ labelKey: "presetCoast", url: "https://videos.pexels.com/video-files/2169880/2169880-hd_1920_1080_30fps.mp4" },
		...
		const en = {
			...
			"dynbg.auroraDesc": "Flowing polar lights",
				...
				"dynbg.presetCoast": "Tropical coast",
		...
		function preloadVideo(url) {
			...
			// clock; only a source that stops delivering data for 15s is reported.
			const armStallTimer = () => {
				if (entry.timer !== null) clearTimeout(entry.timer);
				entry.timer = setTimeout(() => settle(() => reportVideoError?.("timeout")), 15000);
			};
			armStallTimer();
