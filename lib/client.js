		...
		const WALLPAPERS = [
			...
			},
			{
				...
				descKey: "auroraDesc",
					...
					ctx.fillStyle = sky;
					ctx.fillRect(0, 0, w, h);
					// Twinkling background stars (only above the horizon).
					const ns = Math.round(120 * Math.max(0.6, cfg.density));
					const stars = cfg._astars ??= [];
					while (stars.length < ns) stars.push({ x: Math.random(), y: Math.random() * 0.55, r: 0.4 + Math.random() * 1.1, p: Math.random() * 6.28, s: 0.5 + Math.random() * 1.8 });
					stars.length = ns;
					for (const s of stars) {
						const tw = 0.3 + 0.7 * Math.abs(Math.sin(t * 0.001 * s.s + s.p));
					...
					// Aurora as a field of vertical light rays. For each x column we sample a
					// brightness from layered sine waves and draw a soft vertical gradient streak
					// whose colour shifts green -> teal -> violet with height. Additive blending makes
					// dense regions glow without harsh band edges.
					const horizonY = h * 0.72;
					const curtainTop = h * 0.16;
					// Curtain shape parameters — two slowly drifting wave fields modulate brightness
					// and vertical extent so the aurora breathes and folds instead of sitting flat.
					const field = (x, tt) => {
						const k = x / w;
					...
					for (let x = 0; x < w; x += colW) {
						...
						// height goes 0 (top) -> 1 (bottom): green tip, teal middle, violet base
						grad.addColorStop(0, "rgba(80, 255, 160, 0)");
						grad.addColorStop(0.15, `rgba(90, 255, 180, ${0.06 * bright})`);
						grad.addColorStop(0.45, `rgba(80, 230, 220, ${0.10 * bright})`);
						grad.addColorStop(0.78, `rgba(120, 150, 255, ${0.07 * bright})`);
						grad.addColorStop(1, "rgba(170, 120, 255, 0)");
						ctx.fillStyle = grad;
						ctx.fillRect(x, top, colW + 1, horizonY - top);
					}
					// A brighter concentrated lower edge — the curtain's glowing foot.
					for (let x = 0; x < w; x += colW) {
						const bright = field(x, t * cfg.speed);
						if (bright < 0.45) continue;
						const baseY = horizonY - (8 + reach(x, t * cfg.speed) * 30);
						const glow = ctx.createRadialGradient(x, baseY, 0, x, baseY, 60);
						glow.addColorStop(0, `rgba(110, 255, 190, ${0.12 * bright})`);
						glow.addColorStop(1, "rgba(110, 255, 190, 0)");
						ctx.fillStyle = glow;
						ctx.fillRect(x - 60, baseY - 60, 120, 120);
					}
					ctx.globalCompositeOperation = "source-over";
					// Mountain ranges: three receding layers of smooth sine-summed ridgelines.
					// Each layer is a filled path from a multi-harmonic sine so the silhouettes look
					// like real ridges instead of zig-zag triangles. Far layers are lighter/hazier.
					...
					const rangeCfg = [
						{ base: 0.70, shade: "rgba(10, 22, 44, 0.55)", scale: 0.7 },
						{ base: 0.75, shade: "rgba(6, 14, 30, 0.85)", scale: 1.0 },
						{ base: 0.80, shade: "rgba(2, 6, 16, 1)", scale: 1.25 }
					];
					for (let r = 0; r < 3; r++) {
						const rg = ranges[r];
						const rc = rangeCfg[r];
					...
					for (let x = 0; x < w; x += colW) {
						...
						const grad = ctx.createLinearGradient(x, top, x, horizonY);
						grad.addColorStop(0, "rgba(80, 255, 160, 0)");
						grad.addColorStop(0.45, `rgba(80, 230, 220, ${0.05 * bright})`);
						grad.addColorStop(1, "rgba(170, 120, 255, 0)");
						ctx.fillStyle = grad;
						ctx.fillRect(x, top, colW + 1, horizonY - top);
					}
					...
					const rippleCount = Math.round(24 * cfg.density);
					const ripples = cfg._aripples ??= [];
					while (ripples.length < rippleCount) ripples.push({ x: Math.random(), y: 0.78 + Math.random() * 0.18, len: 0.01 + Math.random() * 0.05, phase: Math.random() * 6.28, alpha: 0.06 + Math.random() * 0.14 });
					ripples.length = rippleCount;
					ctx.lineWidth = 0.8;
