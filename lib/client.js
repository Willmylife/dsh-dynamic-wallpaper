...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const KEY_VIDEOQ = "dsh-dynbg:videoq";
		...
		const WALLPAPERS = [
			{
				...
				descKey: "particlesDesc",
					...
					ctx.globalCompositeOperation = "source-over";
						const n = Math.min(200, Math.round(80 * cfg.density));
					...
					for (let i = 0; i < ps.length; i++) {
						...
						let linksDrawn = 0;
						const maxLinks = 900;
						for (let i = 0; i < ps.length && linksDrawn < maxLinks; i++) {
							const p = ps[i];
							const cellX = Math.floor((p.x * w) / link), cellY = Math.floor((p.y * h) / link);
							for (let offsetX = -1; offsetX <= 1 && linksDrawn < maxLinks; offsetX++) for (let offsetY = -1; offsetY <= 1 && linksDrawn < maxLinks; offsetY++) {
			...
			},
			{
				...
				descKey: "matrixDesc",
					...
					ctx.fillRect(0, 0, w, h);
						const cols = Math.min(220, Math.max(8, Math.round((w / 16) * cfg.density)));
					const cw = w / cols;
					const drops = cfg._drops ??= [];
						while (drops.length < cols) drops.push({ y: Math.random() * -30, v: 8 + Math.random() * 12, trail: 4 + ((Math.random() * 8) | 0), pulse: 0 });
					drops.length = cols;
					...
					for (let i = 0; i < cols; i++) {
						...
						d.y += d.v * dt * cfg.speed;
							if (d.y * 14 > h + 40 && Math.random() < 0.02) { d.y = 0; d.trail = 4 + ((Math.random() * 8) | 0); }
...
// Deep polar-night sky with a subtle vertical gradient plus a faint horizon glow.
					...
					for (const s of stars) {
						const tw = 0.3 + 0.7 * Math.abs(Math.sin(t * 0.001 * s.s + s.p));
						// subtle horizontal parallax drift so the starfield doesn't read as nailed down
						const sx = (s.x + Math.sin(t * 0.00003 + s.p) * 0.004) * w;
						if (tw > 0.85 && s.r > 0.95) {
							const x = sx, y = s.y * h;
					...
					const auroraColor = (hh) => {
						...
						// Render the costly, soft light curtains at half resolution, then scale the
						// layer up. The columns are intentionally broad because their glow is smooth.
						const offScale = 0.5;
						const offW = Math.max(1, Math.round(w * offScale));
						const offH = Math.max(1, Math.round(h * offScale));
						let auroraLayer = cfg._aoff;
						if (!auroraLayer || auroraLayer.width !== offW || auroraLayer.height !== offH) {
							const canvas = typeof OffscreenCanvas === "function" ? new OffscreenCanvas(offW, offH) : document.createElement("canvas");
							canvas.width = offW;
							canvas.height = offH;
							auroraLayer = cfg._aoff = { canvas, width: offW, height: offH };
						}
						const auroraCtx = auroraLayer.canvas.getContext("2d");
						auroraCtx.setTransform(offScale, 0, 0, offScale, 0, 0);
						auroraCtx.clearRect(0, 0, w, h);
						auroraCtx.globalCompositeOperation = "lighter";
						const colW = Math.max(3, Math.round(w / 110));
						...
						for (let x = 0; x < w; x += colW) {
							...
							const grad = auroraCtx.createLinearGradient(x, top, x, horizonY);
							...
							auroraCtx.fillRect(x, top, colW + 1, horizonY - top);
						}
						...
						for (let x = 0; x < w; x += colW * 2) {
							...
							const col = auroraColor(hueAt(x, t * cfg.speed));
							const r = 70;
							const glow = auroraCtx.createRadialGradient(x, baseY, 0, x, baseY, r);
							...
							auroraCtx.fillRect(x - r, baseY - r, r * 2, r * 2);
						}
						auroraCtx.globalCompositeOperation = "source-over";
						ctx.save();
						ctx.globalCompositeOperation = "lighter";
						ctx.drawImage(auroraLayer.canvas, 0, 0, offW, offH, 0, 0, w, h);
						ctx.restore();
					// Horizon glow band — warm light spilling above the mountain line where the aurora illuminates the sky.
					const hg = ctx.createLinearGradient(0, horizonY - h * 0.12, 0, horizonY);
					hg.addColorStop(0, "rgba(40, 90, 120, 0)");
					hg.addColorStop(0.7, "rgba(60, 120, 150, 0.06)");
					hg.addColorStop(1, "rgba(80, 150, 170, 0.10)");
					ctx.fillStyle = hg;
					ctx.fillRect(0, horizonY - h * 0.12, w, h * 0.12);
					...
					if (frontPoints.length > 1) {
						...
						// Lake reflection: mirror the pre-rendered aurora below the front range.
						ctx.save();
						ctx.globalCompositeOperation = "lighter";
		...
		// Royalty-free sample videos (Pexels) offered as one-click starting points
		// so the custom-video card is never a blank box. Content verified against
		// each clip's poster frame; capped at 1080p and kept small (≤21MB) so the
		// first click plays within seconds — 4K or long clips stall on slow links.
		const VIDEO_PRESETS = [
			{ labelKey: "presetWaves", url: "https://videos.pexels.com/video-files/1093662/1093662-hd_1920_1080_30fps.mp4" },
			{ labelKey: "presetConcert", url: "https://videos.pexels.com/video-files/2022395/2022395-hd_1920_1080_30fps.mp4" },
			{ labelKey: "presetEarth", url: "https://videos.pexels.com/video-files/3129957/3129957-hd_1920_1080_25fps.mp4" }
		];
		...
		const zh = {
			...
			"dynbg.auroraDesc": "流动极光帷幕",
				"dynbg.custom": "自定义视频",
				"dynbg.customDesc": "粘贴视频网址或导入本地文件",
				"dynbg.presets": "快速试用：",
				"dynbg.videoQuality": "视频画质：",
				"dynbg.vqSmooth": "流畅",
				"dynbg.vqBalanced": "平衡",
				"dynbg.vqDirect": "高清",
				"dynbg.presetWaves": "海岸浪花",
				"dynbg.presetConcert": "现场演出",
				"dynbg.presetEarth": "数字地球",
			"dynbg.speed": "速度",
			"dynbg.density": "密度",
			"dynbg.opacity": "界面遮罩",
			"dynbg.blur": "模糊",
			...
			"dynbg.urlApply": "应用",
			"dynbg.urlLoading": "加载中…",
			"dynbg.urlImport": "导入文件",
			"dynbg.remove": "移除",
				"dynbg.errorInvalid": "网址格式无效，请输入 http(s) 视频直链（mp4/webm/mov/m4v 等）",
				"dynbg.errorVideo": "视频无法加载，请检查网址或网络连接",
				"dynbg.errorNetwork": "视频请求被拒绝：目标站点可能有防盗链或跨域限制，也可能链接已失效",
				"dynbg.errorDecode": "视频文件无法解码，请换一个来源试试",
				"dynbg.errorFormat": "浏览器不支持该视频格式，推荐 MP4 (H.264) 或 WebM",
				"dynbg.errorTimeout": "视频加载超时，请检查网络后重试",
				"dynbg.errorBlob": "导入的视频无法恢复（浏览器可能处于隐私模式或存储已被清理），请重新导入文件",
		...
		const en = {
			...
			"dynbg.auroraDesc": "Flowing polar lights",
				...
				"dynbg.customDesc": "Paste a video URL or import a local file",
				"dynbg.presets": "Quick try:",
				"dynbg.videoQuality": "Quality:",
				"dynbg.vqSmooth": "Smooth",
				"dynbg.vqBalanced": "Balanced",
				"dynbg.vqDirect": "Sharp",
				"dynbg.presetWaves": "Ocean waves",
				"dynbg.presetConcert": "Live show",
				"dynbg.presetEarth": "Digital earth",
			"dynbg.speed": "Speed",
			...
			"dynbg.urlLoading": "Loading…",
			"dynbg.urlImport": "Import file",
			"dynbg.remove": "Remove",
				"dynbg.errorInvalid": "Invalid URL — enter an http(s) video link (mp4/webm/mov/m4v, etc.)",
				"dynbg.errorVideo": "The video could not be loaded. Check the URL or network connection.",
				"dynbg.errorNetwork": "The video request was rejected — the host may block hotlinking or CORS, or the link is dead.",
				"dynbg.errorDecode": "The video could not be decoded; try another source.",
				"dynbg.errorFormat": "The browser cannot play this format; MP4 (H.264) or WebM works best.",
				"dynbg.errorTimeout": "Timed out loading the video; check the network and try again.",
				"dynbg.errorBlob": "The imported video could not be restored (private browsing or cleared storage). Please import the file again.",
		...
		/** Video wallpaper quality: "smooth" | "balanced" | "direct". */
		function readVideoQ() {
			const raw = readStorage(KEY_VIDEOQ);
			return raw === "smooth" || raw === "direct" ? raw : "balanced";
		}
		function sanitizeVideoUrl(raw) {
			...
			// blob: URLs are generated locally from imported files (Wallpaper Engine videos, etc.)
			if (/^blob:/i.test(value)) return value;
			if (/^data:video\//i.test(value)) return value;
			// Accept http(s) links with a known video extension, or any http(s) link
			// without an obvious non-video resource suffix — playback errors are caught
			// by the <video> error event, so we don't need to be stricter than that.
			if (/^https?:\/\//i.test(value)) {
				const ext = (/\.(mp4|webm|ogv|mov|m4v|mkv)(\?|#|$)/i.exec(value));
				if (ext) return ext[1].toLowerCase() === "mkv" ? null : value;
				// no extension: allow if it doesn't look like a document/image resource
				if (/\.(html?|php|aspx?|jpg|jpeg|png|gif|svg|css|js|json|xml|pdf|zip)(\?|#|$)/i.test(value)) return null;
				return value;
			}
			return null;
		}
		...
		//#region local video store (IndexedDB)
		// Imported files are persisted here so they survive reloads; the blob: URL
		// in localStorage only records that the active wallpaper is a local import
		// (blob URLs themselves die with the page).
		let idbPromise = null;
		function idbOpen() {
			if (idbPromise === null) {
				idbPromise = new Promise((resolve) => {
					let request;
					try { request = indexedDB.open("dsh-dynbg", 1); }
					catch { resolve(null); return; }
					request.onupgradeneeded = () => { try { request.result.createObjectStore("videos"); } catch {} };
		...
		function idbPutVideo(file) {
			return idbRun("readwrite", (store) => store.put(file, "imported"));
		}
		function idbGetVideo() {
			return idbRun("readonly", (store) => store.get("imported"));
		}
		function idbDeleteVideo() {
			return idbRun("readwrite", (store) => store.delete("imported"));
		}
		//#endregion
		...
		// Per-wallpaper draw state (particles, stars, …) lives on this object as
		// `_`-prefixed fields created lazily by each draw(); it is reset whenever
		// the active wallpaper changes so old scenes don't linger in memory.
		let animCfg = { speed: DEFAULT_SPEED, density: DEFAULT_DENSITY };
		let activeWallpaperId = null;
		let overrideDispose = null;
		let lastFrame = 0;
		let activeCtx = null;
		let reportVideoError = null;
		// The blob: URL created by the last local-file import, if any. Only URLs
		// created by this plugin are revoked, never user-typed blob: links.
		...
		// A video being preloaded on a detached element: the current wallpaper keeps
		// rendering until the new source can play, so switching never flashes blank.
		let pendingVideo = null;
		// Active video→canvas blit loop (quality modes below "direct"): the video is
		// kept off-DOM and redrawn into a downscaled canvas at a capped frame rate,
		// which is far cheaper for the compositor than a full-resolution video layer.
		let blitState = null;

		function cancelPendingVideo() {
			if (pendingVideo === null) return;
			const pending = pendingVideo;
			pendingVideo = null;
			if (pending.timer !== null) clearTimeout(pending.timer);
			try { pending.el.pause(); pending.el.removeAttribute("src"); pending.el.load(); } catch {}
		}

		function bindLiveVideoError(video) {
			if (video._dynbgErrBound) return;
			video._dynbgErrBound = true;
			video.addEventListener("error", () => {
				if (layerEl !== video && blitState?.video !== video) return;
				if (readType() !== "custom") return;
				// Ignore errors from a source that was already replaced: when URLs are
				// switched quickly the browser can still fire the old request's error.
				const current = sanitizeVideoUrl(readStorage(KEY_VIDEO));
				const showing = video.currentSrc || video.src || "";
				if (current !== null && showing !== "" && showing !== current) return;
				reportVideoError?.(video.error?.code);
			});
		}

		/** Present a loaded video element as the wallpaper layer in the current
		 *  quality mode (direct DOM layer, or off-DOM blitted to a canvas). */
		function activateVideo(video) {
			teardownLayerEl();
			resetAnimState();
			bindLiveVideoError(video);
			if (readVideoQ() !== "direct") {
				// Blit mode: keep the video off-DOM and present it through a
				// downscaled canvas at a capped frame rate.
				const canvas = document.createElement("canvas");
				canvas.style.cssText = "position:fixed;inset:0;z-index:-1;pointer-events:none;width:100%;height:100%;display:block;";
				const blit = { video, canvas, ctx: canvas.getContext("2d"), interval: 1000 / (readVideoQ() === "smooth" ? 20 : 24), lastDraw: 0, gen: 0 };
				blitState = blit;
				applyBlitSize(blit);
				layerEl = canvas;
		...
		function preloadVideo(url) {
			const wantBlit = readVideoQ() !== "direct";
			if (pendingVideo !== null && pendingVideo.src === url) return;
			if (layerEl && layerEl.tagName === "VIDEO" && document.body.contains(layerEl) && !wantBlit && (layerEl.src === url || layerEl.currentSrc === url)) {
				// Already playing this source: refresh filter/playback state only.
				const blur = readNumber(KEY_BLUR, 0, 60, DEFAULT_BLUR);
				layerEl.style.filter = blur > 0 ? `blur(${blur}px)` : "none";
				if (!document.hidden) {
					const p = layerEl.play();
					if (p && typeof p.catch === "function") p.catch(function () {});
				}
				if (ownedObjectUrl !== null && ownedObjectUrl !== url) releaseOwnedObjectUrl();
				return;
			}
			if (layerEl && layerEl.tagName === "CANVAS" && wantBlit && blitState !== null && layerEl === blitState.canvas && videoSourceOf(blitState.video) === url) {
				// Same source in blit mode: a quality/blur change only resizes the target.
				applyBlitSize(blitState);
				if (ownedObjectUrl !== null && ownedObjectUrl !== url) releaseOwnedObjectUrl();
				return;
			}
			// Same source, different presentation mode: re-present the live element
			// instead of re-fetching the clip over the network.
			if (!wantBlit && blitState !== null && videoSourceOf(blitState.video) === url) {
				activateVideo(blitState.video);
				if (ownedObjectUrl !== null && ownedObjectUrl !== url) releaseOwnedObjectUrl();
				return;
			}
			if (wantBlit && layerEl && layerEl.tagName === "VIDEO" && document.body.contains(layerEl) && (layerEl.src === url || layerEl.currentSrc === url)) {
				const video = layerEl;
				// Detach first so teardownLayerEl inside activateVideo cannot strip its src.
				layerEl = null;
				video.remove();
				activateVideo(video);
				if (ownedObjectUrl !== null && ownedObjectUrl !== url) releaseOwnedObjectUrl();
			...
			el.style.cssText = "position:fixed;inset:0;z-index:-1;pointer-events:none;width:100%;height:100%;object-fit:cover;";
			const entry = { el, src: url, timer: null };
			const settle = (run) => {
				if (pendingVideo !== entry) return;
				pendingVideo = null;
				if (entry.timer !== null) clearTimeout(entry.timer);
				run();
			};
			// Sliding stall timeout: a slow but progressing load keeps resetting the
			// clock. A connection that stops delivering data stalls — transient
			// CDN/proxy hiccups get one silent refetch before the user sees an error.
			const armStallTimer = () => {
				if (entry.timer !== null) clearTimeout(entry.timer);
				entry.timer = setTimeout(() => {
					...
					settle(() => reportVideoError?.("timeout"));
				}, 18000);
			};
			armStallTimer();
			el.addEventListener("progress", () => { if (pendingVideo === entry) armStallTimer(); });
			el.addEventListener("canplay", () => settle(() => {
				// Swap layers only once the new video can actually play.
				activateVideo(el);
				// The previous import is no longer the active source — its buffer can
				// be freed now that the new video is live.
				if (ownedObjectUrl !== null && ownedObjectUrl !== url) releaseOwnedObjectUrl();
			}), { once: true });
			el.addEventListener("error", () => settle(() => reportVideoError?.(el.error?.code)), { once: true });
			pendingVideo = entry;
			el.src = url;
		}
		...
		/** Size the blit canvas: base quality scale, deepened by the blur slider —
		 *  downscaling plus smooth upscaling approximates blur almost for free. */
		function applyBlitSize(blit) {
			const base = readVideoQ() === "smooth" ? 0.5 : 0.75;
			const blur = readNumber(KEY_BLUR, 0, 60, DEFAULT_BLUR);
			const scale = base / (1 + blur / 24);
			const w = Math.max(2, Math.round(window.innerWidth * scale));
			const h = Math.max(2, Math.round(window.innerHeight * scale));
			if (blit.canvas.width !== w || blit.canvas.height !== h) {
				blit.canvas.width = w;
				blit.canvas.height = h;
			}
		}

		function blitFrame() {
			const blit = blitState;
			if (blit === null) return;
			if (layerEl !== blit.canvas || readType() !== "custom") {
				blitState = null;
				return;
			}
			if (!document.hidden) {
				const now = performance.now();
				if (now - blit.lastDraw >= blit.interval) {
					blit.lastDraw = now;
					applyBlitSize(blit);
					const vw = blit.video.videoWidth, vh = blit.video.videoHeight;
					if (vw && vh) {
						const cover = Math.max(blit.canvas.width / vw, blit.canvas.height / vh);
						const sw = blit.canvas.width / cover, sh = blit.canvas.height / cover;
						blit.ctx.drawImage(blit.video, (vw - sw) / 2, (vh - sh) / 2, sw, sh, 0, 0, blit.canvas.width, blit.canvas.height);
					}
				}
				scheduleBlit(blit);
			}
			// While hidden no new callbacks are scheduled; onVisibility restarts them.
		}

		function scheduleBlit(blit) {
			const gen = blit.gen;
			const step = () => { if (blitState === blit && blit.gen === gen) blitFrame(); };
			if (typeof blit.video.requestVideoFrameCallback === "function") blit.video.requestVideoFrameCallback(step);
			else requestAnimationFrame(step);
		}

		let restoreFailedFor = null;
		/** Rebuild the imported-video blob URL from IndexedDB after a reload. */
		function restoreImportedVideo() {
			idbGetVideo().then((file) => {
				if (readType() !== "custom") return;
				if (!file) {
					restoreFailedFor = readStorage(KEY_VIDEO);
					reportVideoError?.();
					return;
				}
				...
				if (readType() === "custom") reportVideoError?.();
			});
		}
		...
		/** Detach the layer video from an imported blob URL, then revoke it. */
		function releaseOwnedObjectUrl() {
			if (ownedObjectUrl === null) return;
			const url = ownedObjectUrl;
			ownedObjectUrl = null;
			try {
				if (layerEl && layerEl.tagName === "VIDEO" && layerEl.src === url) {
					layerEl.removeAttribute("src");
					layerEl.load();
				}
				if (blitState !== null && videoSourceOf(blitState.video) === url) {
					blitState.video.removeAttribute("src");
					blitState.video.load();
				}
			} catch {}
		...
		/** Drop all per-wallpaper draw state (wallpaper off/switch/plugin unload). */
		function resetAnimState() {
			animCfg = { speed: DEFAULT_SPEED, density: DEFAULT_DENSITY };
		...
		function startAnim(ctx) {
			...
			if (!c2d) return;
			const typeId = readType();
			const wp = WALLPAPERS.find((x) => x.id === typeId);
			if (!wp) return;
			const fresh = { speed: readNumber(KEY_SPEED, 0.1, 4, DEFAULT_SPEED), density: readNumber(KEY_DENSITY, 0.2, 3, DEFAULT_DENSITY) };
			if (typeId === activeWallpaperId) {
				// Same wallpaper (slider tweak): keep the seeded scene so it doesn't pop.
				for (const key in animCfg) if (key.startsWith("_")) fresh[key] = animCfg[key];
			}
			// A different wallpaper starts from scratch; the old scene is dropped.
			animCfg = fresh;
			activeWallpaperId = typeId;
			const blur = readNumber(KEY_BLUR, 0, 60, DEFAULT_BLUR);
		...
		function applyWallpaper(ctx) {
			...
			try {
				...
				if (type === "custom") {
					...
					if (url === null) {
						// No usable stored video: drop the bad value but KEEP the custom
						// selection — the card the user just pressed must stay expanded
						// so they can paste a URL or import a file.
					...
					if (/^blob:/i.test(url)) {
						// blob: URLs die with the page — reuse this session's URL when we
						// own it, otherwise rebuild it from the persisted import.
						if (ownedObjectUrl === url) preloadVideo(url);
						else if (restoreFailedFor !== url) restoreImportedVideo();
					} else {
						preloadVideo(url);
					}
				} else {
		...
		function onVisibility() {
			...
			if (blitState !== null) {
				if (document.hidden) blitState.video.pause();
				else {
					const p = blitState.video.play();
					if (p && typeof p.catch === "function") p.catch(function () {});
					// Bump the generation so stale pending callbacks die out.
					blitState.gen++;
					scheduleBlit(blitState);
				}
			}
			if (!document.hidden && activeCtx && layerEl?.tagName === "CANVAS" && blitState === null) scheduleApply(activeCtx);
		}
		function teardown() {
			if (applyRaf !== null) {
			...
			if (pointer._bound) {
				pointer._bound = false;
				window.removeEventListener("pointermove", onPointerMove);
				window.removeEventListener("pointerleave", onPointerLeave);
			}
			if (typeof themeOff === "function") { try { themeOff(); } catch {} }
			themeOff = null;
			teardownLayerEl();
			releaseOwnedObjectUrl();
			resetAnimState();
		...
		const styles = {
			...
			presets: { display: "flex", alignItems: "center", flexWrap: "wrap", gap: "6px" },
			preset: { padding: "4px 10px", borderRadius: "6px", border: "1px solid var(--dsw-alias-border-l2)", background: "var(--dsw-alias-interactive-bg-hover)", color: "var(--dsw-alias-label-secondary)", cursor: "pointer", font: "inherit", fontSize: "12px", boxSizing: "border-box" },
			presetLabel: { color: "var(--dsw-alias-label-tertiary)", fontSize: "12px" }
		};
		...
		function WallpaperRow({ t, setType, setSpeed, setDensity, setOpacity, setBlur, setVideo, setVideoQ, useStore }) {
			const type = useStore((s) => s.type);
			const speed = useStore((s) => s.speed);
			const density = useStore((s) => s.density);
			const opacity = useStore((s) => s.opacity);
			const blur = useStore((s) => s.blur);
			const videoq = useStore((s) => s.videoq);
			const videoUrl = useStore((s) => s.video);
			const error = useStore((s) => s.error);
				// Start empty so the backfill effect below can substitute the imported
				// file's name for the unreadable blob: URL on first render.
			...
			// Backfill the saved URL whenever the custom tab becomes active or the stored URL changes externally.
			(0, _react.useEffect)(() => {
				if (type !== "custom" || urlInput) return;
				if (videoUrl && videoUrl.startsWith("blob:")) {
					// blob URLs are unreadable — show the imported file's name instead
					let name = null;
					try { name = window.localStorage.getItem("dsh-dynbg:videoname"); } catch {}
					setUrlInput(name ?? videoUrl);
				} else setUrlInput(videoUrl ?? "");
			}, [type, videoUrl]);
			const applyUrl = (override) => {
				const sanitized = sanitizeVideoUrl(override ?? urlInput);
				if (sanitized === null) {
					// keep the typed value so the user can edit it instead of retyping
			...
			// Import a local video file (e.g. a Wallpaper Engine .mp4 from the Steam workshop directory)
			// via a hidden <input type=file>. The blob URL is saved to localStorage; it survives tab
			// reloads within the same session but not a full page restart, so the filename is stored
			// alongside as a hint for re-import.
			const importFile = (event) => {
				...
				const url = URL.createObjectURL(file);
				// The previous import is being replaced; its blob can be freed right away.
				releaseOwnedObjectUrl();
				ownedObjectUrl = url;
				// Persist the file itself so the import survives reloads.
				idbPutVideo(file);
				setUrlInput(file.name);
				setLoading(true);
				setVideo(url, null);
				// stash the filename so a reload can show what was last imported
				try { window.localStorage.setItem("dsh-dynbg:videoname", file.name); } catch {}
			};
			// Clear loading once a video URL is bound (success) or an error surfaces.
			(0, _react.useEffect)(() => {
				if (videoUrl || error) setLoading(false);
			}, [videoUrl, error]);
			// Safety timeout: if neither success nor error fires (e.g. same URL re-applied), clear loading.
				(0, _react.useEffect)(() => {
					if (!loading) return;
					const id = setTimeout(() => setLoading(false), 4000);
					return () => clearTimeout(id);
				}, [loading]);
				// The settings preview is its own video decoder. Pause it with the main
				// layer when DSH is backgrounded so it cannot consume resources unseen.
				(0, _react.useEffect)(() => {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					type === "custom" ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									(0, react_jsx_runtime.jsx)("span", { style: styles.presetLabel, children: t("dynbg.presets") }),
									...VIDEO_PRESETS.map((preset) => (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										style: styles.preset,
										onClick: () => { setUrlInput(preset.url); applyUrl(preset.url); },
										children: t("dynbg." + preset.labelKey)
									}, preset.url))
								]
							}),
							(0, react_jsx_runtime.jsxs)("div", {
								style: styles.presets,
								children: [
									(0, react_jsx_runtime.jsx)("span", { style: styles.presetLabel, children: t("dynbg.videoQuality") }),
									...[["smooth", "vqSmooth"], ["balanced", "vqBalanced"], ["direct", "vqDirect"]].map(([q, key]) => (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										"aria-pressed": videoq === q,
										style: { ...styles.preset, ...(videoq === q ? styles.cardSelected : {}) },
										onClick: () => setVideoQ(q),
										children: t("dynbg." + key)
									}, q))
								]
							}),
							(0, react_jsx_runtime.jsxs)("div", {
								style: { ...styles.urlRow, flexWrap: "wrap" },
								children: [
									(0, react_jsx_runtime.jsx)("input", { ref: fileRef, type: "file", accept: "video/*,.mp4,.webm,.ogv,.mov,.m4v", style: { display: "none" }, onChange: importFile }),
									(0, react_jsx_runtime.jsx)("input", { type: "text", value: urlInput, placeholder: t("dynbg.urlPlaceholder"), style: styles.urlInput, onChange: (event) => setUrlInput(event.target.value), onKeyDown: (event) => { if (event.key === "Enter" && !loading) applyUrl(); } }),
									(0, react_jsx_runtime.jsx)("button", { type: "button", style: { ...styles.button, opacity: loading ? 0.6 : 1 }, disabled: loading, onClick: () => applyUrl(), children: loading ? t("dynbg.urlLoading") : t("dynbg.urlApply") }),
									(0, react_jsx_runtime.jsx)("button", { type: "button", style: styles.button, onClick: () => fileRef.current?.click(), children: t("dynbg.urlImport") }),
									videoUrl ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, {
										children: [
											(0, react_jsx_runtime.jsx)("video", { ref: previewRef, src: videoUrl, muted: true, loop: true, autoPlay: !document.hidden, playsInline: true, preload: "metadata", referrerPolicy: "no-referrer", style: styles.preview }),
					...
					error ? (0, react_jsx_runtime.jsx)("div", { style: styles.error, children: t(error === "save" ? "dynbg.errorSave" : error === "invalid" ? "dynbg.errorInvalid" : error === "network" ? "dynbg.errorNetwork" : error === "decode" ? "dynbg.errorDecode" : error === "format" ? "dynbg.errorFormat" : error === "timeout" ? "dynbg.errorTimeout" : videoUrl?.startsWith("blob:") ? "dynbg.errorBlob" : "dynbg.errorVideo") }) : null,
		...
		function createWallpaperStore() {
			return (0, _runtime_client.defineStore)({
				init: () => ({ type: DEFAULT_TYPE, speed: DEFAULT_SPEED, density: DEFAULT_DENSITY, opacity: DEFAULT_OPACITY, blur: DEFAULT_BLUR, videoq: "balanced", video: null, error: null, revision: -1 }),
				actions: {
					sync: (d, type, speed, density, opacity, blur, videoq, video, error, revision) => {
		...
		function apply(ctx) {
			...
			const sync = () => {
				revision += 1;
				bound?.sync(readType(), readNumber(KEY_SPEED, 0.1, 4, DEFAULT_SPEED), readNumber(KEY_DENSITY, 0.2, 3, DEFAULT_DENSITY), readNumber(KEY_OPACITY, 0, 1, DEFAULT_OPACITY), readNumber(KEY_BLUR, 0, 60, DEFAULT_BLUR), readVideoQ(), sanitizeVideoUrl(readStorage(KEY_VIDEO)), rowError, revision);
			};
			reportVideoError = (code) => {
				rowError = code === "timeout" ? "timeout" : code === 2 ? "network" : code === 3 ? "decode" : code === 4 ? "format" : "video";
				sync();
			};
			...
			themeOff = ctx.on("theme/change", (snapshot) => {
			...
			const injected = (actions) => {
				...
				setType: (type) => {
					...
					if (!writeStorage(KEY_TYPE, type === DEFAULT_TYPE ? null : type)) {
						rowError = "save";
						sync();
						return;
					}
				...
				setSpeed: (value) => {
					if (!writeStorage(KEY_SPEED, String(Math.min(4, Math.max(0.1, value))))) {
						rowError = "save";
						sync();
						return;
					}
				...
				setDensity: (value) => {
					if (!writeStorage(KEY_DENSITY, String(Math.min(3, Math.max(0.2, value))))) {
						rowError = "save";
						sync();
						return;
					}
				...
				setOpacity: (percent) => {
					if (!writeStorage(KEY_OPACITY, String(Math.min(1, Math.max(0, percent / 100))))) {
						rowError = "save";
						sync();
						return;
					}
				...
				setBlur: (px) => {
					if (!writeStorage(KEY_BLUR, String(Math.min(60, Math.max(0, px))))) {
						rowError = "save";
						sync();
						return;
					}
				...
				setVideoQ: (q) => {
					if (q !== "smooth" && q !== "balanced" && q !== "direct") return;
					// "balanced" is the default — keep storage clean by storing nothing.
					if (!writeStorage(KEY_VIDEOQ, q === "balanced" ? null : q)) {
						rowError = "save";
						sync();
						return;
					}
				...
				setVideo: (url, error) => {
					if (url === null) {
						const removed = writeStorage(KEY_VIDEO, null);
						let cleared = true;
						if (error === null && readType() === "custom") cleared = writeStorage(KEY_TYPE, null);
						if (!removed || !cleared) {
							rowError = "save";
							sync();
							return;
						...
						// The save failed, so nothing will ever reference this import again.
						if (url === ownedObjectUrl) releaseOwnedObjectUrl();
						rowError = "save";
						sync();
						return;
					}
