		...
		const WALLPAPERS = [
			{
				...
				draw(ctx, w, h, t, dt, cfg) {
					...
					while (ps.length < n) ps.push({ x: Math.random(), y: Math.random(), vx: (Math.random() - 0.5) * 0.02, vy: (Math.random() - 0.5) * 0.02, r: 1 + Math.random() * 2 });
					ps.length = n;
					const link = 0.14 * w;
					...
					ctx.strokeStyle = "rgba(142, 166, 216, 0.18)";
					ctx.lineWidth = 1;
			...
			},
			{
				...
				draw(ctx, w, h, t, dt, cfg) {
					...
					const ss = cfg._stars ??= [];
					while (ss.length < n) ss.push({ z: Math.random() });
					ss.length = n;
					...
					for (const s of ss) {
						...
						ctx.fillStyle = `rgba(230, 235, 255, ${0.3 + (1 - s.z) * 0.7})`;
			...
			},
			{
				...
				draw(ctx, w, h, t, dt, cfg) {
					...
					for (let b = 0; b < bands; b++) {
						...
						ctx.lineTo(w, h);
						ctx.closePath();
						ctx.fillStyle = `rgba(64, 128, 224, ${0.12 + b * 0.08})`;
						ctx.fill();
					}
				}
			},
			{
				...
				descKey: "rainDesc",
				draw(ctx, w, h, t, dt, cfg) {
					const n = Math.round(160 * cfg.density);
					const rs = cfg._rain ??= [];
					while (rs.length < n) rs.push({ x: Math.random(), y: Math.random(), v: 0.5 + Math.random() * 0.5, l: 0.04 + Math.random() * 0.06 });
					rs.length = n;
					ctx.fillStyle = "rgba(10, 13, 20, 0.45)";
					ctx.fillRect(0, 0, w, h);
					ctx.strokeStyle = "rgba(160, 190, 230, 0.4)";
					ctx.lineWidth = 1;
					for (const r of rs) {
						r.y += r.v * dt * cfg.speed;
						if (r.y > 1.1) { r.y = -0.1; r.x = Math.random(); }
						const x = r.x * w, y = r.y * h, l = r.l * h;
						ctx.beginPath();
						ctx.moveTo(x, y);
						ctx.lineTo(x - 2, y - l);
						ctx.stroke();
			...
			},
			{
				...
				draw(ctx, w, h, t, dt, cfg) {
					const grad = ctx.createLinearGradient(0, 0, 0, h);
					grad.addColorStop(0, "#071a24");
					grad.addColorStop(1, "#0a2b38");
					ctx.fillStyle = grad;
					ctx.fillRect(0, 0, w, h);
					...
					for (const b of bs) {
						...
						ctx.strokeStyle = "rgba(140, 220, 235, 0.35)";
						ctx.lineWidth = 1;
						ctx.stroke();
						ctx.fillStyle = "rgba(140, 220, 235, 0.08)";
						ctx.fill();
					}
				}
			},
			{
				...
				draw(ctx, w, h, t, dt, cfg) {
					...
					for (let i = 0; i < cols; i++) {
						...
						ctx.fillStyle = Math.random() < 0.08 ? "#c8ffd8" : "rgba(60, 220, 120, 0.8)";
