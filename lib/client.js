	...
	factory: (require) => {
		...
		const WALLPAPERS = [
			...
			},
			{
				...
				descKey: "auroraDesc",
				draw(ctx, w, h, t, dt, cfg) {
					// Deep polar-night sky with a subtle vertical gradient.
					const sky = ctx.createLinearGradient(0, 0, 0, h);
					sky.addColorStop(0, "#02030f");
					sky.addColorStop(0.55, "#061026");
					sky.addColorStop(1, "#0a1a30");
					ctx.fillStyle = sky;
					ctx.fillRect(0, 0, w, h);
					// Twinkling background stars.
					const ns = Math.round(120 * Math.max(0.6, cfg.density));
					const stars = cfg._astars ??= [];
					while (stars.length < ns) stars.push({ x: Math.random(), y: Math.random() * 0.6, r: 0.4 + Math.random() * 1.1, p: Math.random() * 6.28, s: 0.5 + Math.random() * 1.8 });
					stars.length = ns;
					for (const s of stars) {
						const tw = 0.3 + 0.7 * Math.abs(Math.sin(t * 0.001 * s.s + s.p));
					...
					// Aurora curtains: several layered flowing bands built from stacked sine ribbons.
					// Each band is a vertical column of colour whose top edge undulates; additive blending
					// makes overlapping bands glow like real polar lights.
					ctx.globalCompositeOperation = "lighter";
					const bandCount = Math.max(3, Math.round(5 * cfg.density));
					const bands = cfg._abands ??= [];
					while (bands.length < bandCount) bands.push({
						yBase: 0.18 + Math.random() * 0.28,
						amp: 0.05 + Math.random() * 0.10,
						freq: 1.5 + Math.random() * 2.5,
						speed: 0.00015 + Math.random() * 0.0003,
						phase: Math.random() * 6.28,
						height: 0.25 + Math.random() * 0.30,
						hue: Math.random() < 0.4 ? "90, 255, 180" : Math.random() < 0.6 ? "120, 200, 255" : "190, 130, 255",
						alpha: 0.10 + Math.random() * 0.10
					});
					bands.length = bandCount;
					for (const b of bands) {
						const phase = t * b.speed * cfg.speed + b.phase;
						const step = Math.max(6, Math.round(w / 120));
						// Build the curtain as a filled ribbon: top edge undulates, bottom fades out.
						ctx.beginPath();
						ctx.moveTo(0, h);
						for (let x = 0; x <= w; x += step) {
							const k = (x / w) * b.freq * Math.PI;
							const top = b.yBase + Math.sin(k + phase) * b.amp + Math.sin(k * 0.5 - phase * 0.7) * b.amp * 0.4;
							ctx.lineTo(x, top * h);
						}
						// bottom edge mirrors with a slower wave so the curtain has thickness
						for (let x = w; x >= 0; x -= step) {
							const k = (x / w) * b.freq * Math.PI;
							const bot = b.yBase + b.height + Math.sin(k * 0.8 + phase * 1.1) * b.amp * 0.6;
							ctx.lineTo(x, bot * h);
						}
						ctx.closePath();
						const cy = (b.yBase + b.height * 0.5) * h;
						const grad = ctx.createLinearGradient(0, b.yBase * h, 0, (b.yBase + b.height) * h);
						grad.addColorStop(0, `rgba(${b.hue}, 0)`);
						grad.addColorStop(0.4, `rgba(${b.hue}, ${b.alpha})`);
						grad.addColorStop(0.7, `rgba(${b.hue}, ${b.alpha * 0.7})`);
						grad.addColorStop(1, `rgba(${b.hue}, 0)`);
						ctx.fillStyle = grad;
						ctx.fill();
						// vertical rays rising from the curtain's bright band
						ctx.strokeStyle = `rgba(${b.hue}, ${b.alpha * 0.4})`;
						ctx.lineWidth = 0.8;
						for (let x = 0; x <= w; x += step * 3) {
							const k = (x / w) * b.freq * Math.PI;
							const top = b.yBase + Math.sin(k + phase) * b.amp;
							ctx.beginPath();
							ctx.moveTo(x, top * h);
							ctx.lineTo(x, (top - 0.08) * h);
							ctx.stroke();
						}
					}
					ctx.globalCompositeOperation = "source-over";
					// Distant mountain silhouette at the horizon.
					const horizonY = h * 0.72;
					ctx.fillStyle = "rgba(5, 10, 22, 0.95)";
					ctx.beginPath();
					ctx.moveTo(0, horizonY);
					const peaks = cfg._apeaks ??= [];
					while (peaks.length < 16) peaks.push({ x: peaks.length / 16, h: 0.03 + Math.random() * 0.09 });
					for (const p of peaks) {
						const px = p.x * w;
						// sharpen peaks into ridgelines
						ctx.lineTo(px - 8, horizonY - p.h * h * 0.5);
						ctx.lineTo(px, horizonY - p.h * h);
						ctx.lineTo(px + 8, horizonY - p.h * h * 0.5);
					}
					ctx.lineTo(w, horizonY);
					ctx.lineTo(w, h); ctx.lineTo(0, h); ctx.closePath();
					ctx.fill();
					// Lake reflection: the aurora faintly mirrors into the water below the ridgeline.
					ctx.save();
					ctx.globalCompositeOperation = "lighter";
					...
					for (const b of bands) {
						...
						grad.addColorStop(0, `rgba(${b.hue}, ${b.alpha * 0.5})`);
						grad.addColorStop(1, `rgba(${b.hue}, 0)`);
						ctx.fillStyle = grad;
						ctx.fill();
					}
					ctx.restore();
					// Ripple lines on the water surface.
					const ripples = cfg._aripples ??= [];
					while (ripples.length < Math.round(24 * cfg.density)) ripples.push({ x: Math.random(), y: 0.74 + Math.random() * 0.22, len: 0.01 + Math.random() * 0.05, phase: Math.random() * 6.28, alpha: 0.06 + Math.random() * 0.14 });
					ripples.length = Math.round(24 * cfg.density);
					ctx.lineWidth = 0.8;
		...
		function CardChip({ id }) {
			const backgrounds = {
				...
				bubbles: "linear-gradient(180deg, #071a24, #0a3a4a)",
					...
					aurora: "linear-gradient(180deg, #04102a 30%, #0a3a4a 70%, #072a3a)"
			};
