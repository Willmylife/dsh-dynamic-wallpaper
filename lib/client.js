...
// Deep polar-night sky with a subtle vertical gradient plus a faint horizon glow.
		...
		const zh = {
			...
			"dynbg.errorBlob": "导入的文件已失效（页面刷新后需重新导入），请点击「导入文件」重新选择",
		...
		const en = {
			...
			"dynbg.errorBlob": "The imported file is no longer available (it must be re-imported after a page reload). Click \"Import file\" to select it again.",
		...
		function WallpaperRow({ t, setType, setSpeed, setDensity, setOpacity, setBlur, setVideo, useStore }) {
			...
			(0, _react.useEffect)(() => {
				if (type === "custom" && !urlInput) setUrlInput(videoUrl ?? "");
			}, [type, videoUrl]);
			const applyUrl = () => {
